...
export default function CategoryManager() {
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      ...
      <section className="space-y-3 rounded-lg border bg-white p-5 shadow-sm">
        <h2 className="text-lg font-semibold text-gray-900">Add Subcategory</h2>
        <form onSubmit={handleSubcategorySubmit} className="space-y-3">
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">Subcategory name</label>
            <input
      ...
      <section className="md:col-span-2 rounded-lg border bg-white p-5 shadow-sm">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          {categories.length === 0 ? (
            ...
            categories.map((c) => (
              <div key={c.id} className="rounded border bg-gray-50 p-3">
                <p className="font-semibold text-gray-900">{c.name}</p>
                <div className="mt-2 flex flex-wrap gap-2">
                  {subcategories.filter((s) => s.categoryId === c.id).map((s) => (
                    <span key={s.id} className="rounded bg-white px-2 py-1 text-xs text-gray-700 shadow-sm">
                      {s.name}
                    </span>
                  ))}
